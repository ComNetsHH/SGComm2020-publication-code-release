...
TdBetFfMacScheduler::EstimateUlSinr (uint16_t rnti, uint16_t rb)
{
  ...
  if (itCqi == m_ueCqi.end ())
  ...
  else
    {
      ...
      for (uint32_t i = rb; i < uint32_t(rb+2); i++)
        {
...
TdBetFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  if (rbPerFlow < 2)
    {
      rbPerFlow = 2;  // at least 3 rbg per flow (till available resource) to ensure TxOpportunity >= 7 bytes
    }
  int rbAllocated = 4;
  ...
  do
    {
      ...
      NS_LOG_INFO (this << " RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows << " ulBandwidth " <<  int(m_cschedCellConfig.m_ulBandwidth));
      while ((!allocated)&&((rbStart + rbPerFlow - m_cschedCellConfig.m_ulBandwidth) < 1) && (rbPerFlow != 0))
        {
          // check availability
          bool free = true;
          for (uint16_t j = rbStart; j < rbStart + rbPerFlow; j++)
            {
              if (rbMap.at (j) == true)
                {
                  free = false;
