...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    .AddAttribute ("RemoteAddressTrace", "The destination address of the outbound packets for trace identification",
                   AddressValue (),
                   MakeAddressAccessor (&UdpClient::m_peerAddressTrace),
                   MakeAddressChecker ())
    ...
    .AddTraceSource ("TxWithAddress",
                   "A packet has been sent",
                   MakeTraceSourceAccessor (&UdpClient::m_txTraceWithAddress),
  ...
  if ((m_socket->Send (p)) >= 0)
    {
      ...
      m_txTraceWithAddress (p, m_peerAddressTrace);
    }
  else
    {
